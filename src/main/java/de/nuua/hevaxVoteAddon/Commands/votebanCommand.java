...
public class votebanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        currentVoteId = "vote_ban_" + System.currentTimeMillis();
        ...
        Bukkit.broadcastMessage("§8");
        Bukkit.broadcastMessage("§8  | §7Eine Abstimmung zum §fBann §7von §b" + targetPlayer.getName() + " §7wurde gestartet!");
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (countdown <= 0) {
                    ...
                    Bukkit.broadcastMessage("§8 ◇§m------------------§8» §b§lᴀʙѕᴛɪᴍᴍᴜɴɢ§8 §8«§m------------------§8◇");
                    Bukkit.broadcastMessage("§8");
                    Bukkit.broadcastMessage("§8  | §7Eine Abstimmung zum §fBann §7von §b" + targetPlayer.getName() + " §7läuft!");
    ...
    private void endVote() {
        ...
        Bukkit.broadcastMessage("§8 ◇§m------------------§8» §b§lᴀʙѕᴛɪᴍᴍᴜɴɢ§8 §8«§m------------------§8◇");
        Bukkit.broadcastMessage("§8");
        Bukkit.broadcastMessage("§8  | §7Eine Abstimmung zum §fBann §7von §b" + targetPlayer + "§f");
        ...
        if (yesVotes > noVotes) {
            ...
            if (player != null) {
                ...
                long banEndTime = System.currentTimeMillis() + 600000; // 10 minutes in milliseconds
                config.set("banned_players." + player.getUniqueId(), banEndTime);
                plugin.saveConfig();

                player.kickPlayer("§cDu wurdest durch eine Abstimmung für 10 Minuten gebannt.\n§7Grund: §f" + reason);

                Bukkit.broadcastMessage("§8  | §f" + targetPlayer + " §7wurde für §b10 Minuten §agebannt§f.");
            } else {
